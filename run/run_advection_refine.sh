...
SCRATCH=$GLOBALSCRATCH/murphy-adv-refine-${TAG}
...
while [ $i -le 4 ]
do
    export NCPUS_TMP=$(echo "$(( ($i+1)*96 ))")
    export NCPUS=$(echo "$(( $NCPUS_TMP > 0 ? $NCPUS_TMP : 96))")
	# get the folder
	export RUN_DIR=$SCRATCH/level$i

    #--------------------------------------------------------------------------
    # start form level 0, and eps_r/eps_c = 16
	export NO_ADAPT="--iter-adapt=6"
	export ILEVEL=0
    export RTOL=$(bc -l <<< "scale=16; 10^(-${i}-2)")
    export CTOL=$(bc -l <<< "scale=16; 10^(-${i}-3)")
