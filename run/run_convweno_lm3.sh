...
cp -r $HOME_MURPHY/make_arch/make.lm3 $SCRATCH/$RUN_DIR/make_arch
...
do
    ...
    do
        ...
        #compilation 
        GIT_COMMIT=${GITCOMMIT} ARCH_FILE=make_arch/make.lm3 make -j OPTS="-DWAVELET_N=${N} -DWAVELET_NT=${NT} -DBLOCK_GS=${COUNT_GS}"
        # move to the full name
        MURPHY_NAME=murphy_${N}_${NT}
        # weno 3
        cp murphy ${MURPHY_NAME}
        mpirun --mca pml ucx --mca osc ucx --mca btl ^uct ./${MURPHY_NAME} --conv-weno --ilevel=4 --level-max=9 > log_${N}${NT}_$SLURM_JOB_ID.log
        # weno 5
        ## conservative 3
        ...
        GIT_COMMIT=${GITCOMMIT} ARCH_FILE=make_arch/make.lm3 make destroy
