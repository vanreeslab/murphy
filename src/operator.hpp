 ...
 */
template <class T>
void Operator(const op_t<T> op, Grid* grid, Field* field, T data) {
    m_begin;
 ...
 */
template <class T>
void Operator(const const_op_t<T> op, Grid* grid, const Field* field, T data) {
    m_begin;
    m_assert(grid->is_mesh_valid(), "mesh is not valid, unable to process");
    //-------------------------------------------------------------------------
    // get the grid info
 ...
 */
template <class T>
void Operator(const bop_t<T> op, Grid* grid, Field* field, T data) {
    m_begin;
    //-------------------------------------------------------------------------
    // get the grid info
    p8est_t* forest = grid->forest();
