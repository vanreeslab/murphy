...
void ErrorCalculator::Norms(Grid* grid, const Field* field, const Field* sol, Field* error, real_t* norm_2, real_t* norm_i) {
    m_begin;
 ...
 */
void ErrorCalculator::ErrorFieldOnGridBlock(const qid_t* qid, GridBlock* block, const Field* fid, const Field* sol,const Field* error) {
    //-------------------------------------------------------------------------
    const real_t* hgrid = block->hgrid();
    ...
    for (sid_t ida = 0; ida < fid->lda(); ++ida) {
        ...
        const data_ptr data_field = block->data(fid, ida);
        const data_ptr data_sol   = block->data(sol, ida);
        data_ptr       data_error = block->data(error, ida);
        m_assume_aligned(data_field);
        m_assume_aligned(data_sol);
        m_assume_aligned(data_error);
        // get the correct place given the current thread and the dimension
        for (lid_t i2 = start_; i2 < end_; i2++) {
            for (lid_t i1 = start_; i1 < end_; i1++) {
                for (lid_t i0 = start_; i0 < end_; i0++) {
