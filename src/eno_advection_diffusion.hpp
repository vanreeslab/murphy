#ifndef SRC_ENO_ADVECTION_DIFFUSION_HPP_
#define SRC_ENO_ADVECTION_DIFFUSION_HPP_
...
class ENO_AdvectionDiffusion : public Stencil {
   protected:
    real_t nu_          = 0.0;
    real_t u_stream_[3] = {0.0, 0.0, 1.0};

   public:
    explicit ENO_AdvectionDiffusion(const real_t nu, const real_t u_stream[3]) : Stencil() {
        //-------------------------------------------------------------------------
        nu_ = nu;
        for (lda_t i = 0; i < 3; ++i) {
            u_stream_[i] = u_stream[i];
        }
        m_log("ENO Advection Diffusion with u stream = %e %e %e and nu = %e", u_stream_[0], u_stream_[1], u_stream_[2], nu_);
        //-------------------------------------------------------------------------
    }
   ...
   protected:
    void DoMagic(const qid_t* qid, GridBlock* block, const bool is_outer, Field* fid_src, Field* fid_trg) override;
};

template <sid_t length_advection, sid_t length_diffusion>
void ENO_AdvectionDiffusion<length_advection, length_diffusion>::DoMagic(const qid_t* qid, GridBlock* block, const bool is_outer, Field* fid_src, Field* fid_trg) {
