...
//==============================================================================
template <>
lid_t Advection<M_ADV_CENTER, 2>::NGhost() const { return 1; };
template <>
...
//==============================================================================
template <>
lid_t Advection<M_ADV_CENTER, 4>::NGhost() const { return 2; }
template <>
...
//==============================================================================
template <>
lid_t Advection<M_ADV_CENTER,6>::NGhost() const { return 3; }
template <>
...
void Advection<M_ADV_WENO_VEL, 3>::DoMagic(m_ptr<const qid_t> qid, m_ptr<GridBlock> block, const bool is_outer, m_ptr<const Field> fid_src, m_ptr<Field> fid_trg) const {
    ...
    auto op = [=, &data_trg](const bidx_t i0, const bidx_t i1, const bidx_t i2) -> void {
        ...
        trg[0] -= 0.5 * (1.0 + m_sign(w[0])) * flux[5] + 0.5 * (1.0 - m_sign(w[0])) * flux[5];
    };
...
template <>
lid_t Advection<M_ADV_WENO_VEL, 5>::NGhost() const { return 3; }
