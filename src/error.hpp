...
class ErrorCalculator : public BlockOperator{
   ...
   public:
    ...
    void Norms(Grid* grid, const Field* field, const Field* sol, Field* error, real_t* norm_2, real_t* norm_i);

    void ErrorOnGridBlock(const qid_t* qid, GridBlock* block, const Field* fid, const Field* sol);
    void ErrorFieldOnGridBlock(const qid_t* qid, GridBlock* block, const Field* fid, const Field* sol, const Field* error);
};
