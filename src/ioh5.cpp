...
void IOH5::operator()(ForestGrid* grid, Field* field, string name) {
    ...
    block_stride_ = (dump_ghost_) ? M_STRIDE : M_N;
    ...
    //................................................
    // print the header
    xmf_write_header_(grid, field->lda());
...
void IOH5::hdf5_write_header_(const ForestGrid* grid, const hsize_t n_block_global, const hsize_t lda) {
    m_begin;
    //-------------------------------------------------------------------------
    //................................................
    ...
    hsize_t memsize[3] = {M_STRIDE * lda, M_STRIDE, M_STRIDE};  // m_blockmemsize(lda);
...
void IOH5::xmf_write_header_(const ForestGrid* grid, const lda_t lda) {
    m_begin;
    //-------------------------------------------------------------------------
    rank_t rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    ...
    int quad_len = grid->forest()->local_num_quadrants * len_per_quad_;
    int pos_end = header_count + quad_len;

    // gt count, the position of proc i (in lines!!) as the sum of the position of procs 0 -> (i-1)
    int pos_curr = 0;
    MPI_Scan(&pos_end, &pos_curr, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
...
size_t IOH5::xmf_core_(const string fname_h5, const real_t* hgrid, const real_t* xyz, const p4est_topidx_t tid, const p4est_locidx_t qid, const rank_t rank, const lid_t stride, const lid_t n_gs, const lda_t lda, const hsize_t offset, const hsize_t stride_global, char* msg) {
    ...
    for (lda_t ida = 0; ida < lda; ++ida) {
        ...
        strcat(msg, line);
        // - L13
        sprintf(line, "                  <DataItem ItemType=\"HyperSlab\" Dimensions=\"%20.20lld %10.10d %10.10d\" Type=\"HyperSlab\">\n", stride, stride, stride);
        strcat(msg, line);
        // - L13
        ...
        strcat(msg, line);
        // - L13
        sprintf(line, "                     <DataItem Dimensions=\"%10.10d %10.10d %10.10d\" NumberType=\"Float\" Precision=\"4\" Format=\"HDF\">\n", stride_global, stride, stride);
        strcat(msg, line);
        // - L14
