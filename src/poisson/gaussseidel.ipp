...
void GaussSeidel<length>::IterativeSolverInner(const qid_t* qid, GridBlock* block, Field* fid_sol, Field* fid_rhs, Field* fid_tmp) {
    m_assert(fid_tmp == nullptr, "No temp array needed for GS");
    //-------------------------------------------------------------------------
    // note: for a not very clear reason, the splitting inner-outer is unstable in GS, so everything is performed on the outerloop
    /*
...
template <sid_t length>
void GaussSeidel<length>::IterativeSolverOuter(const qid_t* qid, GridBlock* block, Field* fid_sol, Field* fid_rhs,Field* fid_tmp) {
    m_assert(fid_tmp == nullptr, "No temp array needed for GS");
    //-------------------------------------------------------------------------
    // get the laplacian stencil and scale factor
    real_t* coef_lapla = coef_lapla_ + (length / 2);
    ...
    real_t  scale[3]   = {1.0 / pow(block->hgrid(0), 2), 1.0 / pow(block->hgrid(1), 2), 1.0 / pow(block->hgrid(2), 2)};
    // compute GS coefficients
    real_t alpha = 1.0*alpha_;
    real_t coef_gs[length];
    real_t denom    = -coef_lapla[0] * (scale[0] + scale[1] + scale[2]);
    real_t coef_rhs = alpha / denom;
    for (sid_t is = -(length / 2); is <= (length / 2); is++) {
        coef_gs[(length / 2) + is] = coef_lapla[is] * coef_rhs;
    }
    // get the correct pointer and override the central coefficient
