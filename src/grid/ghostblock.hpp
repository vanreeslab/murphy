...
class GhostBlock : public SubBlock {
   protected:
    ...
    bidx_t ghost_len_[2]       = {0, 0};   //!< length of the ghost region in front at the back of the block

   public:
    GhostBlock() = delete;
    GhostBlock(const bidx_t gs, const bidx_t stride) : SubBlock(gs, stride){};
    ...
    void ghost_len(const bidx_t ghost_len[2]) {
