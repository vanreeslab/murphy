...
Partitioner::Partitioner(map<string, Field* > *fields, Grid *grid, bool destructive) {
    ...
    if (nqshipped > 0) {
        ...
        if (opart_n > 0) {
            ...
            send_buf_.Allocate(opart_n * PartCommSize(n_lda_));
            // if (destructive_) {
            //     // the status are only send if the partitioner is destructive
            ...
            for (rank_t ir = 0; ir < n_recver; ++ir) {
                ...
                real_t* __restrict buf = send_buf_.ptr + qcount * PartCommSize(n_lda_);
        ...
        if (cpart_n > 0) {
            ...
            recv_buf_.Allocate(cpart_n * PartCommSize(n_lda_));
            ...
            for (rank_t ir = 0; ir < n_sender; ir++) {
                ...
                // create the send request
                real_t* __restrict buf = recv_buf_.ptr + qcount * PartCommSize(n_lda_);
    ...
    for (rank_t is = 0; is < n_send_request; ++is) {
        ...
        for (rank_t iq = 0; iq < n_q2send; ++iq) {
            ...
            for (const auto iter : *fields) {
                ...
                memcpy(lbuf, block->RawPointer(fid,0), CartBlockMemNum(fid->lda()) * sizeof(real_t));
                // update the counters
                idacount += fid->lda();
            }
        }
    ...
    for (rank_t ir = 0; ir < n_recv_request; ++ir) {
        ...
        for (iblock_t iq = 0; iq < n_q2recv; ++iq) {
            ...
            for (auto iter : *fields) {
                ...
                real_p lbuf = buf + CartBlockMemNum(idacount);
                // copy the whole memory at once on the dim
                memcpy(block->RawPointer(fid, 0), lbuf, CartBlockMemNum(fid->lda()) * sizeof(real_t));
                // update the counters
                idacount += fid->lda();
            }
