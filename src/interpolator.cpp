...
void Interpolator::Interpolate(const sid_t dlvl, const lid_t shift[3], MemLayout *block_src, real_p data_src, MemLayout *block_trg, real_p data_trg)
{
    ...
    Interpolate(dlvl, shift, block_src, data_src, block_trg, data_trg, 0.0, data_trg, nullptr);
}
 ...
 * @param block_trg descripiton of data_trg
 * @param data_trg the 0-position of the trg memory, i.e. the memory location of (0,0,0) for the target
 * @param normal the normal of the ghost layer to compute, dictate which direction is affected by the jump in resolution
 */
void Interpolator::Interpolate(const sid_t dlvl, const lid_t shift[3], MemLayout *block_src, real_p data_src, MemLayout *block_trg, real_p data_trg, sid_t normal[3])
{
    // if not normal we simply set a nullptr
    Interpolate(dlvl, shift, block_src, data_src, block_trg, data_trg, 0.0, data_trg, normal);
}
 ...
 */
void Interpolator::Interpolate(const sid_t dlvl, const lid_t shift[3], MemLayout *block_src, real_p data_src, MemLayout *block_trg, real_p data_trg, const real_t alpha, real_p data_cst, sid_t normal[3])
{
    ...
    if (dlvl == -1 && normal == nullptr)
    {
        m_log("the normal is = %p, so going to refine as usual", normal);
        Refine_(&ctx);
    }
    else if (dlvl == -1 && normal != nullptr)
    {
        m_log("the normal is = %p, so going to refine go ghosts", normal);
        RefineGhost_(&ctx);
    }
    else if (dlvl == 0)
    {
