...
real_t OddBoundary_4::Stencil_(const real_t* f, const real_t x, const real_t h, const real_t offset, const real_t normal, const real_t value) {
    ...
    real_t cor_f[3] = {f[0] - value,
                       ...
                       f[2] - value};
    // compute the stencil coefficients
    const real_t a = (10.0 * cor_f[0] - 5.0 * cor_f[1] + 1.0 * cor_f[2]) / (60.0 * pow(h, 5));
    const real_t b = (-34.0 * cor_f[0] + 13.0 * cor_f[1] - 1.0 * cor_f[2]) / (24.0 * pow(h, 3));
    const real_t c = (2250.0 * cor_f[0] - 125.0 * cor_f[1] + 9.0 * cor_f[2]) / (960.0 * pow(h, 1));
...
real_t ExtrapBoundary_4::Stencil_(const real_t* f, const real_t x, const real_t h, const real_t offset, const real_t normal, const real_t value) {
    m_assert(offset == (0.5*h), "this function is only for cell-centered ghosts");
    // compute the stencil coefficients
    const real_t a = (-1.0 * f[0] + 3.0 * f[1] - 3.0 * f[2] + 1.0 * f[3]) / (6.0 * pow(h, 3));
    const real_t b = (5.0 * f[0] - 13.0 * f[1] + 11.0 * f[2] - 3.0 * f[3]) / (4.0 * pow(h, 2));
    const real_t c = (-71.0 * f[0] + 141.0 * f[1] - 93.0 * f[2] + 23.0 * f[3]) / (24.0 * pow(h, 1));
    const real_t d = (35.0 * f[0] - 35.0 * f[1] + 21.0 * f[2] - 5 * f[3]) / (16.0 * pow(h, 0));
    // return the polynomial + the flux slope
