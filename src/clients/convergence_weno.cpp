    ...
    for (level_t il = 0; il < N_PT; ++il) {
        ...
        // set the test field
        {
            lambda_setvalue_t lambda_cos = [=](const bidx_t i0, const bidx_t i1, const bidx_t i2, const CartBlock* const block, const Field* const fid) -> void {
                ...
                block->data(fid, 0).Write(i0, i1, i2)[0] = std::cos(2.0 * M_PI * freq[0] / L[0] * pos[0]) +
            ...
            // SetValue     field_init(lambda_cos,ghost_len);
            SetValue field_init(lambda_cos);
            field_init(&grid, &test);

            // const bidx_t ghost_len[2] = {3, 3};
            // grid.GhostPull(&test, ghost_len);
            // lambda_error_t lambda_err_cos = [=](const bidx_t i0, const bidx_t i1, const bidx_t i2, const CartBlock* const block) -> real_t {
            //     real_t pos[3];
            //     block->pos(i0, i1, i2, pos);

            //     return 1.0;// std::cos(2.0 * M_PI * freq[0] / L[0] * pos[0]) +
            //         //    std::cos(2.0 * M_PI * freq[0] / L[0] * pos[1]) +
            //         //    std::cos(2.0 * M_PI * freq[0] / L[0] * pos[2]);
            // };
            ...
            // real_t erri, err2;
            // error.Norms(&grid, &test, &lambda_err_cos, &err2, &erri);
            // m_log("after ghosting: %e %e", err2, erri);
