...
TEST_F(valid_Wavelet, coarsen_order_3) {
    for (int id = 0; id < 3; id++) {
        coarse_start_[id] = 0;
        coarse_end_[id]   = M_HN;
        fine_start_[id]   = -M_GS;
        ...
        for (int i2 = fine_start_[2]; i2 < fine_end_[2]; i2++) {
            for (int i1 = fine_start_[1]; i1 < fine_end_[1]; i1++) {
                for (int i0 = fine_start_[0]; i0 < fine_end_[0]; i0++) {
                    ...
                    data_fine_[m_midx(i0, i1, i2, 0, block_fine_)] = (order3_int(x_right) - order3_int(x_left)) / hfine_;
                }
            }
        }
        ...
        for (int i2 = coarse_start_[2]; i2 < coarse_end_[2]; i2++) {
            for (int i1 = coarse_start_[1]; i1 < coarse_end_[1]; i1++) {
                for (int i0 = coarse_start_[0]; i0 < coarse_end_[0]; i0++) {
                    ...
                    ASSERT_NEAR(data_coarse_[m_midx(i0, i1, i2, 0, block_coarse_)], (order3_int(x_right) - order3_int(x_left)) / hcoarse_, DOUBLE_TOL) << "while testing direction " << id;
                }
            }
        }
    }
